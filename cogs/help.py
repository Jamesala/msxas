...
class Help(Cog):
    ...
    async def help_menu(self, ctx):
        main = Embed(
            title="📔 Menu Pomocy",
            description=f"\n **Przydatne**\n"
                        f"\n **W dzień Mixów otwieramy kanał kolejki – każdy może dołączyć. Jeśli będzie więcej niż 12 graczy, stworzymy dodatkowe kolejki. Mapa jest losowana, a po meczu drużyny głosują na zwycięzców, by rozpocząć kolejny mecz. Bot liczy wygrane i przegrane mecze i tworzy ranking graczy.**\n"
                        f"\n 🌐 [Strona](https://marvelrivals.com.pl)"
